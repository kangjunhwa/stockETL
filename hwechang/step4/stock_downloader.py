...
# yfinance는 최소5분 단위의 주식정보를 제공하는듯함. 우선 1일 단위 고정으로 진행 
# 특정 이유로인해 다운로드가 실패할 경우를 대비하여 재시도 코드입력
...
def process_ticker(ticker, previous_date):
    ...
    os.makedirs(date_dir, exist_ok=True)
		...
		# 생성될 파일명 지정
    file_name = f"{date_dir}/stock_{ticker}_{formatted_end_time}.csv"
    ...
    try:
        ...
        df = download_stock_data(ticker, formatted_start_time, formatted_end_time, interval='1d')
...
if __name__ == "__main__":
		# 실행시 매개변수 2개를 입력해야 실행되도록
    if len(sys.argv) < 3:
        print("Usage: python stock_downloader.py <previous_date> <ticker>")
        sys.exit(1)
    
    previous_date = sys.argv[1]
    ticker = sys.argv[2]
    process_ticker(ticker, previous_date)
