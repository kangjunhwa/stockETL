...
VALUES ('0', '$HOST', '$ticker', 'CSV', 0, 'warning', 'file_already_exist', '${previous_date} 00:00:00', '$(date +'%Y-%m-%d %H:%M:%S')', '$job_start_time', '$job_end_time', '$elapsed_time', $filesize);
EOF
    elif [ "$csv_result" == "no_data" ]; then
        psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -U "$DB_USER" << EOF
INSERT INTO etl_logs (log_type, host, ticker, step, data_rows, status, message, from_time, to_time, start_time, end_time, elapsed_time, filesize)
VALUES ('0', '$HOST', '$ticker', 'CSV', 0, 'failed', 'no_data', '${previous_date} 00:00:00', '$(date +'%Y-%m-%d %H:%M:%S')', '$job_start_time', '$job_end_time', '$elapsed_time', $filesize);
EOF
    else
        psql -h "$DB_HOST" -p "$DB_PORT" -d "$DB_NAME" -U "$DB_USER" << EOF
INSERT INTO etl_logs (log_type, host, ticker, step, data_rows, status, message, from_time, to_time, start_time, end_time, elapsed_time, filesize)
VALUES ('0', '$HOST', '$ticker', 'CSV', '$csv_result', 'success', 'save', '${previous_date} 00:00:00', '$(date +'%Y-%m-%d %H:%M:%S')', '$job_start_time', '$job_end_time', '$elapsed_time', $filesize);
EOF
    fi
done < "$ticker_file"
